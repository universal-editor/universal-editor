...
| settings[readonly] | bool или function | Параметр отвечает за указание активности компонента с точки зрения взаимодействия с пользователем  (см. [подробнее](visualization.md). | - | false |
| settings[useable] | function | Параметр отвечает отображение элемента и отправку его значения на сервер  (см. [подробнее](visualization.md). | - | - |
| settings[disabled] | bool | Параметр отвечает за возможность изменения значения поля и его отображение при создании/редактировании. (см. [подробнее](visualization.md) | - | false |
