...
| settings[readonly] | bool или function | Параметр отвечает за указание активности компонента с точки зрения взаимодействия с пользователем  (см. [подробнее](useable.md). | - | false |
| settings[useable] | function | Параметр отвечает отображение элемента и отправку его значения на сервер  (см. [подробнее](useable.md). | - | - |
